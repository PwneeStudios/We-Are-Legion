...
PixelToFrame FragmentShader(VertexToPixel psin)
{
    ...
    if (GpuSim__SimShader__Something(data_here))
    {
        ...
        if (GpuSim__SimShader__IsUnit(unit_here) && (abs(0 - 0.0) < .001 || abs(unit_here.g - 0) < .001) && (!(fs_param_only_selected) || GpuSim__SimShader__selected(data_here)))
        {
            output.xyz = GpuSim__SimShader__pack_coord_3byte(1);
        }
        if (abs(unit_here.r - 0.007843138) < .001 && GpuSim__SimShader__IsCenter(data_here) && abs(unit_here.g - 0) < .001 && (!(fs_param_only_selected) || GpuSim__SimShader__selected(data_here)))
        {
