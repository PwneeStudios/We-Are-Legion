    ...
    return React.createClass({
        ...
        renderAt: function() {
            ...
            var messages = _.map(this.state.messages, function(message) {
                return (
                    React.createElement(ChatLine, {hasBackground: hasBackground, willFadeOut: willFadeOut, key: message.index, message: message, remove: self.remove})
                );
            });

            if (this.props.full) {
                return (
                    React.createElement("div", {ref: "chatDiv", className: "chat-background", style: {'width':'100%','overflow-y':'scroll','height':'100%','pointer-events':'auto'}}, 
                        messages
                    )
                );
            } else {
