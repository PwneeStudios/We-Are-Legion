...
PixelToFrame FragmentShader(VertexToPixel psin)
{
    ...
    if (geo_here.r > 0 + .001 && Game__SimShader__IsBlockingTile__Game_tile(tex2D(fs_param_Tiles, psin.TexCoords + (Game__SimShader__dir_to_vec__float(0.01176471)) * fs_param_Tiles_dxdy)))
    {
        output = dirward_here;
    ...
    {
        if (Game__SimShader__ValidDirward__Game_dirward(forward) && all(abs(forward.rg - geo_forward.ba) < .001))
        {
            output = forward;
        ...
        {
            if (Game__SimShader__ValidDirward__Game_dirward(forward_right) && all(abs(forward_right.rg - geo_forward_right.ba) < .001))
            {
            ...
            {
                if (Game__SimShader__ValidDirward__Game_dirward(forward_left) && all(abs(forward_left.rg - geo_forward_left.ba) < .001))
                {
                    output = forward_left;
                    output.b += 0.003921569;
                }
