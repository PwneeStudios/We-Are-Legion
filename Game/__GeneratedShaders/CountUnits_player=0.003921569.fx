...
PixelToFrame FragmentShader(VertexToPixel psin)
{
    ...
    if (Game__SimShader__Something__Game_data(data_here))
    {
        ...
        bool valid = (abs(0.003921569 - 0.0) < .001 || abs(unit_here.g - 0.003921569) < .001) && (!(fs_param_only_selected) || Game__SimShader__fake_selected__Game_data(data_here));
        if (Game__SimShader__IsUnit__Game_unit(unit_here) && valid)
        {
            output.xyz = Game__SimShader__pack_coord_3byte__float(1);
        }
        if (abs(unit_here.r - 0.02352941) < .001 && Game__SimShader__IsCenter__Game_building(data_here) && valid)
        {
