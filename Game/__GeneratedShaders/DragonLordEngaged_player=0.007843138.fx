...
// Texture Sampler for fs_param_PathToOtherTeams, using register location 3
float2 fs_param_PathToOtherTeams_size;
float2 fs_param_PathToOtherTeams_dxdy;

Texture fs_param_PathToOtherTeams_Texture;
sampler fs_param_PathToOtherTeams : register(s3) = sampler_state
{
    texture   = <fs_param_PathToOtherTeams_Texture>;
    MipFilter = Point;
    MagFilter = Point;
...
PixelToFrame FragmentShader(VertexToPixel psin)
{
    ...
    if (Game__SimShader__Something__data(data_here) && abs(unit_here.g - 0.007843138) < .0019 && abs(unit_here.r - 0.007843138) < .0019 && value < 0.1803922 - .0019)
    {
