...
bool Game__SimShader__IsBlockingTile__Game_tile(float4 t)
{
...
PixelToFrame FragmentShader(VertexToPixel psin)
{
    ...
    if (Game__SimShader__IsBlockingTile__Game_tile(here))
    {
        __FinalOutput.Color = float4(0, 0, 0, 0);
        return __FinalOutput;
    }
    float4 output = geo_here;
    if (!((Game__SimShader__IsBlockingTile__Game_tile(right) && Game__SimShader__IsBlockingTile__Game_tile(left))) && (abs(geo_here.r - 0.007843138) < .001 && abs(geo_up.r - 0.01568628) < .001 || abs(geo_here.r - 0.01568628) < .001 && abs(geo_down.r - 0.007843138) < .001))
    {
        output.r = Game__SimShader__IsBlockingTile__Game_tile(right) ? 0.01176471 : 0.003921569;
    }
    if (!((Game__SimShader__IsBlockingTile__Game_tile(up) && Game__SimShader__IsBlockingTile__Game_tile(down))) && (abs(geo_here.r - 0.003921569) < .001 && abs(geo_right.r - 0.01176471) < .001 || abs(geo_here.r - 0.01176471) < .001 && abs(geo_left.r - 0.003921569) < .001))
    {
        output.r = Game__SimShader__IsBlockingTile__Game_tile(up) ? 0.01568628 : 0.007843138;
    }
    if (abs(tex2D(fs_param_Geo, psin.TexCoords + (Game__SimShader__dir_to_vec__float(output.r)) * fs_param_Geo_dxdy).a - 0.003921569) < .001 && abs(geo_here.a - 0.0) < .001)
    {
        output.r = Game__SimShader__Reverse__float(output.r);
    }
    int surround_count = (Game__SimShader__IsBlockingTile__Game_tile(up) ? 1 : 0) + (Game__SimShader__IsBlockingTile__Game_tile(left) ? 1 : 0) + (Game__SimShader__IsBlockingTile__Game_tile(down) ? 1 : 0) + (Game__SimShader__IsBlockingTile__Game_tile(right) ? 1 : 0);
