...
PixelToFrame FragmentShader(VertexToPixel psin)
{
    ...
    if (!((Game__SimShader__IsBlockingTile__tile(right) && Game__SimShader__IsBlockingTile__tile(left))) && (abs(geo_here.r - 0.007843138) < .001 && abs(geo_up.r - 0.01568628) < .001 || abs(geo_here.r - 0.01568628) < .001 && abs(geo_down.r - 0.007843138) < .001))
    {
        output.r = Game__SimShader__IsBlockingTile__tile(right) ? 0.01176471 : 0.003921569;
    }
    if (!((Game__SimShader__IsBlockingTile__tile(up) && Game__SimShader__IsBlockingTile__tile(down))) && (abs(geo_here.r - 0.003921569) < .001 && abs(geo_right.r - 0.01176471) < .001 || abs(geo_here.r - 0.01176471) < .001 && abs(geo_left.r - 0.003921569) < .001))
    {
        output.r = Game__SimShader__IsBlockingTile__tile(up) ? 0.01568628 : 0.007843138;
    }
    if (abs(tex2D(fs_param_Geo, psin.TexCoords + (Game__SimShader__dir_to_vec__Single(output.r)) * fs_param_Geo_dxdy).a - 0.003921569) < .001 && abs(geo_here.a - 0.0) < .001)
    {
