...
PixelToFrame FragmentShader(VertexToPixel psin)
{
    ...
    float4 info = tex2D(fs_param_Info, psin.TexCoords + (-float2(0.25,0.25) + float2(0, 0)) * fs_param_Info_dxdy);
    float2 pos = psin.TexCoords * fs_param_Info_size;
    if (Game__MakeSymmetricBase__DoNothing__Sampler__vec2__Single(psin, fs_param_Info, fs_param_Info_size, fs_param_Info_dxdy, pos, 0))
    {
        __FinalOutput.Color = info;
        return __FinalOutput;
    }
    float4 copy = tex2D(fs_param_Info, psin.TexCoords + (-float2(0.25,0.25) + float2(0, 0) - Game__MakeSymmetricBase__QuadMirrorShift__Sampler__vec2__Single(psin, fs_param_Info, fs_param_Info_size, fs_param_Info_dxdy, pos, 0)) * fs_param_Info_dxdy);
