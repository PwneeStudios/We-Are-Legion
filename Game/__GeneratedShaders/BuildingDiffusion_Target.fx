...
PixelToFrame FragmentShader(VertexToPixel psin)
{
    ...
    float4 target = tex2D(fs_param_TargetData, psin.TexCoords + (-float2(0.25,0.25) + float2(0, 0)) * fs_param_TargetData_dxdy);
    if (Game__SimShader__Something__building(building_here) && Game__SimShader__IsBuilding__unit(unit_here))
    {
        target = tex2D(fs_param_TargetData, psin.TexCoords + (-float2(0.25,0.25) + Game__SimShader__center_dir__building(building_here)) * fs_param_TargetData_dxdy);
    }
