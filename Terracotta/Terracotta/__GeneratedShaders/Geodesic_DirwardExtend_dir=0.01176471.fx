...
PixelToFrame FragmentShader(VertexToPixel psin)
{
    ...
    if (abs(0.01176471 - 0.007843138) < .001)
    {
        forward = dirward_up;
    ...
    {
        if (abs(0.01176471 - 0.003921569) < .001)
        {
        ...
        {
            if (abs(0.01176471 - 0.01568628) < .001)
            {
            ...
            {
                if (abs(0.01176471 - 0.01176471) < .001)
                {
    ...
    if (geo_here.r > 0 + .001 && Terracotta__SimShader__IsBlockingTile__Terracotta_tile(tex2D(fs_param_Tiles, psin.TexCoords + (Terracotta__SimShader__dir_to_vec__float(0.01176471)) * fs_param_Tiles_dxdy)))
    {
        output = dirward_here;
    ...
    {
        if (Terracotta__SimShader__ValidDirward__Terracotta_dirward(forward) && all(abs(forward.rg - geo_forward.ba) < .001))
        {
            output = forward;
        ...
        {
            if (Terracotta__SimShader__ValidDirward__Terracotta_dirward(forward_right) && all(abs(forward_right.rg - geo_forward_right.ba) < .001))
            {
            ...
            {
                if (Terracotta__SimShader__ValidDirward__Terracotta_dirward(forward_left) && all(abs(forward_left.rg - geo_forward_left.ba) < .001))
                {
                    output = forward_left;
                    output.b += 0.003921569;
                }
