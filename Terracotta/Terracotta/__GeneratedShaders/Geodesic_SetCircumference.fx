...
void Terracotta__SimShader__set_circumference__Terracotta_geo_info__float(inout float4 info, float circumference)
{
    info.ba = Terracotta__SimShader__pack_val_2byte__float(circumference);
}
...
PixelToFrame FragmentShader(VertexToPixel psin)
{
    ...
    float2 start_pos = Terracotta__SimShader__geo_pos_id__Terracotta_geo(here);
    ...
    {
        circum = max(circum, Terracotta__SimShader__polar_dist__Terracotta_geo_info(tex2D(fs_param_Info, psin.TexCoords + (GeoStart + float2(1, 0)) * fs_param_Info_dxdy)));
    }
    if (all(abs(up.gba - here.gba) < .001))
    {
        circum = max(circum, Terracotta__SimShader__polar_dist__Terracotta_geo_info(tex2D(fs_param_Info, psin.TexCoords + (GeoStart + float2(0, 1)) * fs_param_Info_dxdy)));
    }
    ...
    Terracotta__SimShader__set_circumference__Terracotta_geo_info__float(info_here, circum);
